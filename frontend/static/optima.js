/**
 * ╔══════════════════════════════════════════════════════════════════╗
 * ║                   OPTIMA — APPLICATION LOGIC v0.6                ║
 * ║                                                                  ║
 * ║  All UI behavior, API calls, chart rendering, and panel logic.  ║
 * ║  Depends on: optima_schema.js (loaded first), Plotly CDN        ║
 * ║                                                                  ║
 * ║  v0.6: Grouped workspace layout (Counterparty | Economic |      ║
 * ║        Dates) matching business team's Deloitte single deal UI.  ║
 * ║        Results strip with CCY selector, Long/Short Term split.   ║
 * ╚══════════════════════════════════════════════════════════════════╝
 */
...
const BULK_SUPPORTED = ['fx_forward']; // instruments that support bulk upload
...
async function processBulkFile(file) {
  ...
  const previewDiv = $('bulk-results-preview');
  statusDiv.style.display = '';
  statusDiv.className = 'bulk-status loading';
  statusDiv.textContent = `⏳ Uploading and pricing ${file.name} ...`;
  ...
  try {
    ...
    const url = `${API}/pricing/bulk-upload?domestic_rate=${rd}&foreign_rate=${rf}`;
    ...
    a.download = match ? match[1] : `Optima_BulkResults_${file.name}`;
    ...
    statusDiv.textContent = `✓ Bulk pricing complete — results downloaded as ${a.download}`;
  } catch (err) {
...
function openWorkspace(instType, preset) {
  ...
  // Show bulk upload only for fx_forward
  const bulkEl = $('ws-bulk-upload');
  const bulkStatus = $('bulk-status');
  if (bulkEl) bulkEl.style.display = instType === 'fx_forward' ? '' : 'none';
  ...
  if (modeToggle) {
    modeToggle.style.display = BULK_SUPPORTED.includes(instType) ? '' : 'none';
  }
  // Always reset to single deal mode
  ...
  // Check if instrument has grouped layout
  const grouped = typeof FIELD_GROUPS !== 'undefined' && FIELD_GROUPS[instType];
...
function buildGroupedFields(instType, config) {
  ...
  config.groups.forEach(group => {
    ...
    div.className = 'field-group' + (group.fullWidth ? ' full-width' : '');
    ...
    if (group.fullWidth && group.layout) {
      // Full-width row layout (e.g. market data)
...
function collectPayload() {
  ...
  const grouped = typeof FIELD_GROUPS !== 'undefined' && FIELD_GROUPS[instType];
  const params = {};
  const allFields = {};  // ALL collected values (for debugging)

  // Metadata fields that should NOT be sent to the pricing engine
  ...
  if (grouped) {
    ...
    // Build pricing params — only fields the backend expects
    Object.entries(allFields).forEach(([k, v]) => {
      if (!METADATA_KEYS.has(k)) params[k] = v;
    });

    // Map grouped field names to backend-expected names
    ...
    // FX Forward backend expects delivery_date, not expiry
    if (allFields.maturity_date) params.delivery_date = allFields.maturity_date;
  } else {
    // Collect from flat FIELDS
  ...
  // For grouped layout, spot/vol/rate are inside the grouped fields
  ...
  underlyings[underlying] = {
    spot: parseFloat(spotVal) || 100,
    vol: parseFloat(volVal) || 0.25,
    div_yield: parseFloat(divVal) || 0,
  };

  // Rate — from grouped market_data group or flat md fields
  const rateVal = allFields.rate || (($('f-md-rate') || {}).value) || 0.065;
  const foreignVal = allFields.foreign_rate || (($('f-md-foreign_rate') || {}).value) || 0.045;
  const rateCurve = [{ tenor: '1Y', rate: parseFloat(rateVal) || 0.065 }];

  // Pricing date — reporting_date from grouped, or md-pricing_date from flat
  ...
  const payload = {
    ...
    market_data: {
      ...
      foreign_rate: parseFloat(foreignVal) || 0.045,
    },
    model: $('f-model').value,
...
$('btn-price').addEventListener('click', async () => {
  ...
  try {
    ...
    const npv = priceRes.npv;
    $('npv-value').textContent = '$' + npv.toLocaleString(undefined, { minimumFractionDigits: 4, maximumFractionDigits: 4 });
    ...
    // Update results strip if visible
    if ($('rs-npv') && $('ws-results-strip')?.style.display !== 'none') {
      $('rs-npv').textContent = npv.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 });
      $('rs-npv').style.color = npv >= 0 ? 'var(--green)' : 'var(--red)';

      // Long/short term split based on maturity
      ...
      if (matEl && matEl.value) {
        ...
        if (monthsToMaturity > 12) {
          $('rs-long-term').textContent = npv.toLocaleString(undefined, { maximumFractionDigits: 2 });
          $('rs-short-term').textContent = '0.00';
        } else {
          $('rs-long-term').textContent = '0.00';
          $('rs-short-term').textContent = npv.toLocaleString(undefined, { maximumFractionDigits: 2 });
        }
      }
    }
