...
def generate_results_excel(results, errors, upload_filename, pricing_method="flat"):
    ...
    spot = (active[0]["spot"] if active else None) or None
    ...
    rows += [
        ...
        ("Total NPV (USD)", sum(r["npv"] for r in active) / spot if spot else "N/A"),
...
async def bulk_price_json(
    ...
    Frontend calls this to show results table; calls /bulk-upload for Excel.
    """
    if not file.filename.endswith((".xlsx", ".xls")):
        raise HTTPException(400, "Only .xlsx/.xls files accepted")
    ...
    try:
        ...
        def _fmt(r):
            return {
                ...
                "forward":     round(r.get("forward_rate", 0), 4),
                "npv":         round(r["npv"], 2),
